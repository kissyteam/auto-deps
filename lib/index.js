...
function handleBowerJson(srcFile){
	if(fs.existsSync(srcFile)){
		var bowerJson = JSON.parse(fs.readFileSync(srcFile).toString()),
			dependencies = bowerJson.dependencies || (bowerJson.dependencies = { "modulex": "modulex#master" });
		dependencies.modulex = 'modulex#master';
...
function handlePackageJson(srcFile){
	if(fs.existsSync(srcFile)){
		var packageJson = JSON.parse(fs.readFileSync(srcFile).toString()),
			dependencies = packageJson.dependencies || (packageJson.dependencies = { "modulex": "1.3.1" });
		dependencies.modulex = 'modulex#master';
